...
import '../../domain/cartilla_observaciones_campo_config.dart';
import '../../domain/cartilla_observaciones_campo_payload.dart';

class CartillaObservacionesCampoFormState {
  final int localId;
  final bool loading;
  final bool saving;
  final CartillaObservacionesCampoPayload payload;
  final List<String> errors;

  const CartillaObservacionesCampoFormState({
  ...
  CartillaObservacionesCampoFormState copyWith({
    bool? loading,
    bool? saving,
    CartillaObservacionesCampoPayload? payload,
    List<String>? errors,
  }) {
    return CartillaObservacionesCampoFormState(
      localId: localId,
...
final cartillaObservacionesCampoFormProvider =
    StateNotifierProvider.family<
      CartillaObservacionesCampoFormNotifier,
      CartillaObservacionesCampoFormState,
      int
    >((ref, localId) {
      final local = ref.read(registrosLocalDSProvider);
      return CartillaObservacionesCampoFormNotifier(
        ref: ref,
        localId: localId,
        local: local,
      )..load();
...
class CartillaObservacionesCampoFormNotifier
    extends StateNotifier<CartillaObservacionesCampoFormState>
    with GeoSaveMixin
  ...
  CartillaObservacionesCampoFormNotifier({
    ...
    required this.local,
         ...
         CartillaObservacionesCampoFormState(
           ...
           loading: true,
           saving: false,
           payload: CartillaObservacionesCampoPayload.empty(),
           errors: const [],
         ),
       );
  ...
  Future<void> load() async {
    ...
    try {
      ...
      if (isEmptyJson) {
        ...
        payload = CartillaObservacionesCampoPayload.fromJsonString(raw);
      }
  ...
  void update(CartillaObservacionesCampoPayload payload) {
    state = state.copyWith(payload: _recompute(payload), errors: const []);
  }

  CartillaObservacionesCampoPayload _recompute(
    CartillaObservacionesCampoPayload p,
  ) {
  ...
  Future<int> duplicateAsNew() async {
    await saveLocal();
    final cfg = CartillaObservacionesCampoConfig();
  ...
  void updateDataJson(Map<String, dynamic> next) {
    final payload = CartillaObservacionesCampoPayload(
      payloadVersion:
